...
export class VariosComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.productoService.getPedidos().subscribe((data: Orden[]) => {
  ...
  getProductos() {
    this.productoService.getProductos().subscribe((productos) => {
      ...
      this.productos.sort(
        (a: { id_prod: number }, b: { id_prod: number }) =>
          a.id_prod - b.id_prod
      );
    });
  }
  hasProduct(id: any): boolean {
    return this.productos.some(
      (product: { id_prod: number }) => product.id_prod === id
    );
  }
  getOrdenesByVendor() {
    this.productoService.getPedidosByVendor().subscribe((data) => {
      this.ordenesByV = Object.entries(data).map(([vendedorId, productos]) => ({
        vendedorId,
        productos
      }));
  
      this.productArray = this.ordenesByV;
    });
    // this.productoService.getPedidosByVendor().subscribe((data) => {
    ...
    //     console.log('esto devuelve las ordenes');
    //   });
    // });
  }
  vendorName(id: any) {
  ...
  filtrarPorFecha() {
    ...
    if (this.fechaInicio && this.fechaFin) {
      let inicio = new Date(
        ...
        this.fechaInicio.month - 1,
        this.fechaInicio.day
      );
      let fin = new Date(
        this.fechaFin.year,
        this.fechaFin.month - 1,
        this.fechaFin.day
      );
      fin.setHours(23, 59, 59);
      if (inicio.getTime() <= fin.getTime()) {
        this.productoService
          .getPedidosByVendorAndDate(inicio, fin)
          .subscribe((dato) => {
            this.ordenesByV = Object.entries(dato).map(([vendedorId, productos]) => ({
              vendedorId,
              productos
            }));
        ...
        console.log(
          'la fecha de inicio tiene que ser antes que la fecha de final'
        );
      }
    } else {
  ...
  goProductos() {
    this.router.navigate(['admin', this.administrador.id_usu, 'home'], {
      replaceUrl: true,
    });
  }

  goUsuarios() {
    this.router.navigate(['admin', this.administrador.id_usu, 'usuarios'], {
      replaceUrl: true,
    });
  }

  goPedidos() {
    this.router.navigate(['admin', this.administrador.id_usu, 'pedidos'], {
      replaceUrl: true,
    });
  }
  ...
  goVarios() {
    this.router.navigate(
      ['admin', this.administrador.id_usu, 'reporte-varios'],
      { replaceUrl: true }
    );
  }
  ...
  goGlobal() {
    this.router.navigate(
      ['admin', this.administrador.id_usu, 'reporte-global'],
      { replaceUrl: true }
    );
  }
  ...
  goCFProducto() {
    this.router.navigate(
      ['admin', this.administrador.id_usu, 'control-fisico-producto'],
      { replaceUrl: true }
    );
  }
  ...
  goCFEnvase() {
    this.router.navigate(
      ['admin', this.administrador.id_usu, 'control-fisico-envase'],
      { replaceUrl: true }
    );
  }

  goDirecciones() {
    this.router.navigate(['admin', this.administrador.id_usu, 'direcciones'], {
      replaceUrl: true,
    });
  }
